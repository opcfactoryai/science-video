...
# 1. TTS 合成（--text 直接读取 Step 3 生成的 narration.txt）
...
# 2. 自动对齐时间戳 → 输出 scene_boundaries.json
PYTHONPATH="$PWD/.claude/skills/video-script-generator/scripts" \
...
TTS 生成 MP3 + `timestamps.json`，随后立即对齐输出 `scene_boundaries.json`（每镜头的 `start_ms` / `end_ms` / `duration_ms`），供后续分镜图片生成和视频合成使用。
...
- [ ] **所有 prompt 锁死 aspect_ratio + quality** — 每个 video_prompt 和 image_prompt 末尾必须有 `, aspect ratio 16:9/9:16, quality 2K/1K/4K`，100% 覆盖
