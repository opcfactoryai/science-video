...
|------|-------------|-------------|------|
| 横屏（YouTube、B站） | `16:9` | `1K` | 快速出图，可手动调至 2K/4K |
| 竖屏（抖音、Shorts） | `9:16` | `1K` | 快速出图，可手动调至 2K/4K |

- 画幅比例和画质等级**只存放于** `production_notes` 中，不放在脚本根层级
- `production_notes.aspect_ratio` 写入画幅比例（如 `"16:9"` 或 `"9:16"`）
- `production_notes.quality` 写入画质等级（`"1K"` / `"2K"` / `"4K"`），**默认 `"1K"`**（快速出图节省成本）；如需高精度可手动改为 `"2K"` 或 `"4K"`
- `aspect_ratio` 和 `quality` 是工程参数，由 Python 脚本通过 API `size` 参数传入，**不需要写入每个提示词**
- 提示词应锁死的是：**视觉风格**、**光影调性**、**构图逻辑**、**叙事氛围**——这些才是 AI 出图质量的关键
- LLM 不支持自定义分辨率（如 1920×1080），但脚本会自动映射 `production_notes` 中的 `aspect_ratio + quality` 为正确像素尺寸
...
| 镜头类型 | 用途 | 推荐时长 | 叙事功能 |
|----------|------|----------|----------|
| **cover** | 封面/片头 | 1s（封面卡短暂停留后直接进hook） | 建立基调、展示标题，快速过渡至首镜 |
...
Magazine cover level typography, cinematic lighting, premium photorealistic style."
```
...
| **Aspect Ratio** | 🔒 全片锁定（工程参数） | 由 production_notes + Python 脚本自动处理 |
| **Quality / Resolution** | 🔒 全片锁定（工程参数） | 由 production_notes + Python 脚本自动通过 API size 处理 |
...
在 `image_prompt` 和 `video_prompt` 的结尾统一追加风格锚点（锁风格/光影/叙事逻辑，不锁技术规格）：
...
追加统一的后缀锚点，保证全片风格一致（锁风格/光影/构图，不锁工程参数）：
```
, cinematic photorealistic style, dramatic lighting, cinematic composition
```
> **注意：** 画幅 (`aspect_ratio`) 和画质 (`quality`) 是工程参数，由 Python 脚本通过 API `size` 参数传入。提示词中**不需要**追加这些。提示词锁死的是视觉风格、光影调性、构图逻辑这些影响画面审美的要素。
...
深色背景，火光照亮残骸边缘。电影级照片写实风格，超精细细节。冲击、损失、后悔的情绪基调。"
```
...
- [ ] **画质等级已确认** — quality 写入 production_notes（默认 1K），可按需改为 2K/4K
- [ ] **提示词锁风格不锁尺寸** — 画幅和画质由 production_notes + Python 脚本处理，提示词只锁风格、光影、叙事逻辑
